---
name: gstack-browse
description: "Fast headless browser (~100ms/command) for QA testing and site dogfooding. 50+ commands, persistent sessions, snapshot/ref system, diff detection, annotated screenshots."
...
## Cookie Sharing with Nanobot

gstack browse and nanobot's Playwright browser tools can share cookies bidirectionally:

- **gstack -> nanobot**: After importing cookies via `$B cookie-import-browser`, dump them for nanobot:
  ```bash
  $B cookies > ~/.nanobot/workspace/browser_cookies/_gstack.json
  ```
  Nanobot's `browser_load_session` and `browser_open` will auto-detect and load these cookies.

- **nanobot -> gstack**: After `browser_login` saves a session, nanobot writes cookies to
  `~/.nanobot/workspace/browser_cookies/_for_gstack.json`. Import them into gstack:
  ```bash
